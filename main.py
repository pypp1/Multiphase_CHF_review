...
def _result_row(model: str, q_chf_W: float, margin: float, extra: str = ""):
    status = "✓ SAFE" if margin >= 1.0 else "✗ DRYOUT"
    print(f"  {model:<22}  q_CHF = {q_chf_W/1e6:7.3f} MW/m²  "
...
def _run_steady(case, props):
    ...
    print(f"\n  P = {case.P_PA/1e6:.2f} MPa  |  G = {case.G_KGM2S:.0f} kg/(m²·s)  "
          f"|  d = {case.D_M*1000:.0f} mm  |  L = {case.L_M:.2f} m  "
          f"|  x_in = {case.X_IN:.3f}")
    print(f"  q_nom = {case.Q_NOM_MW:.2f} MW/m²\n")
    ...
    if getattr(case, 'RUN_GROENEVELD', False):
        r = groeneveld_chf(case.G_KGM2S, case.D_M, case.L_M,
                           case.P_PA, props.H_fg, x_in=case.X_IN)
        if not np.isnan(r.q_chf):
            _result_row("Groeneveld LUT (2006)", r.q_chf, r.q_chf / q_nom)
        else:
            print("  Groeneveld LUT (2006)  CHF solver did not converge.")
        _warn(r.warnings)
...
def _run_lofa(case, props):
    _header(case.NAME)
    print(f"  {case.DESCRIPTION}")
    print(f"\n  P = {case.P_PA/1e6:.2f} MPa  |  q = {case.Q_NOM_MW} MW/m²  "
          f"|  d = {case.D_M*1000:.0f} mm  |  L = {case.L_M:.2f} m")
    print(f"  G sweep: {case.G_START:.0f} → {case.G_END:.0f} step {case.G_STEP:.0f} kg/(m²·s)\n")
        ...
        for name, _ in models:
            try:
                if name == "Katto":
                    ...
                    r = groeneveld_chf(G, case.D_M, case.L_M,
                                       case.P_PA, props.H_fg, x_in=case.X_IN)
                    q = r.q_chf
                elif name == "Hewitt":
                ...
                flag = " ←DO" if (not np.isnan(margin) and margin < 1.0) else "     "
                row += f"  {margin:>10.3f}{flag}" if not np.isnan(margin) else f"  {'N/A':>14}"
    ...
    for name, _ in models:
        if dryout_G[name] is not None:
            print(f"  ▶ {name}: dryout onset at G = {dryout_G[name]:.0f} kg/(m²·s)")
        else:
...
def _run_pressurisation(case):
    _header(case.NAME)
    print(f"  {case.DESCRIPTION}")
    print(f"\n  G = {case.G_KGM2S:.0f} kg/(m²·s)  |  d = {case.D_M*1000:.0f} mm  "
          f"|  L = {case.L_M:.2f} m")
    print(f"  P:  {case.P_NOM_PA/1e6:.2f} → {case.P_PEAK_PA/1e6:.2f} MPa")
    print(f"  q'': {case.Q_NOM_MW:.2f} → {case.Q_PEAK_MW:.2f} MW/m²\n")
    ...
    print(f"  {'t':>6}  {'P [MPa]':>9}  {'q [MW/m²]':>10}  {hdr_m}")
    print(f"  {'-'*70}")

    for t in t_arr:
        P_t = case.P_NOM_PA  + t * (case.P_PEAK_PA  - case.P_NOM_PA)
        q_t = (case.Q_NOM_MW + t * (case.Q_PEAK_MW  - case.Q_NOM_MW)) * 1e6
        props_t = sat_props(P_t)
        dH_i    = max(0.0, -case.X_IN * props_t.H_fg)
        row = f"  {t:6.3f}  {P_t/1e6:9.3f}  {q_t/1e6:10.4f}"

        for name in models:
            try:
                if name == "Katto":
                    r = katto_chf(case.G_KGM2S, case.D_M, case.L_M,
                                  props_t.rho_l, props_t.rho_v,
                                  props_t.H_fg, props_t.sigma, dH_i=dH_i)
                    q = r.q_c
                elif name == "Biasi":
                    r = biasi_chf(case.G_KGM2S, case.D_M, case.L_M,
                                  P_t, props_t.H_fg, case.X_IN)
                    q = r.q_chf
                elif name == "LUT":
                    r = groeneveld_chf(case.G_KGM2S, case.D_M, case.L_M,
                                       P_t, props_t.H_fg, x_in=case.X_IN)
                    q = r.q_chf
                elif name == "Hewitt":
                    r = hewitt_chf(case.G_KGM2S, case.D_M, case.L_M,
                                   props_t.rho_l, props_t.rho_v,
                                   props_t.H_fg, props_t.sigma,
                                   props_t.eta_l, props_t.eta_v, x_in=case.X_IN)
                    q = r.q_chf

                margin = q / q_t if not np.isnan(q) else float('nan')
                flag = " ←DO" if (not np.isnan(margin) and margin < 1.0) else "     "
                row += f"  {margin:>10.3f}{flag}" if not np.isnan(margin) else f"  {'N/A':>14}"
                if not np.isnan(margin) and margin < 1.0 and dryout_t[name] is None:
                    dryout_t[name] = t
            except Exception:
                row += f"  {'ERR':>14}"
        print(row)
    ...
    print()
    for name in models:
        if dryout_t[name] is not None:
            print(f"  ▶ {name}: dryout onset at t = {dryout_t[name]:.3f}")
        else:
